...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::MainWindow), gw(new GraphWindow(this))
{
    ui->setupUi(this);
    ui->pb_clearResult->setCheckable(true);

    graphClass = new Graphic(gw->ui->qcPlot, 1);

    X.resize(static_cast<uint32_t>(FD));
    Y.resize(static_cast<uint32_t>(FD));

    for(uint32_t i = 0; i < static_cast<uint32_t>(FD);++i)
    {
        X[i] = i;
    }
    connect(this, &MainWindow::sig_GraphDataReady, graphClass, &Graphic::RecieveData);
}
...
QVector<double> MainWindow::ProcessFile(const QVector<uint32_t> dataFile)
{
    ...
    foreach (int word, dataFile) {
        ...
        double res = ((double)word/600000)*10;
...
void MainWindow::on_pb_start_clicked()
{
    ...
    auto findMax = [&](QVector<double> res){
                                                ...
                                                int32_t intFD = static_cast<uint32_t>(FD);

                                                for (uint32_t i = 0; i < intFD;++i)
                                                {
                                                    Y[i] = res[i];
                                                }
                                                emit sig_GraphDataReady(X, Y, gw->ui->qcPlot, 0);
...
{
    graphClass->ClearGraph(gw->ui->qcPlot);
    ui->pb_showGraph->setEnabled(false);
